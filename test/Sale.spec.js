...
describe('Sale', () => {
  ...
  //     await ITO.setPrice(DECIMALS.div(ethers.BigNumber.from(10).pow(4)));
  //     await ITO.addAllow(SeedBuyer1.address);
  //     await ITO.addAllow(SeedBuyer2.address);
  //   });
  ...
  //       const usd = ethers.BigNumber.from(600_000).mul(DECIMALS);
  //       const total = await calcToken(usd);
  ...
  //       it('can claim and unlock 10%', async () => {
  ...
  //           'first lock is 10%').to.be.eq(totalTokenBuyedBySeed1.mul(10).div(100));
  //       });
  ...
  //           const pricePerTranche = totalLockedRemaining.div(9 * 30 * 86400 / (6 * 60 * 60));
  ...
  //             .to.be.eq(totalPreUnlockSeed1);

  //           // 3M + 6h
  //           expect(await ITO.unlockable(SeedBuyer1.address, startClaimTime.add(86400 * 30 * 3 + 6 * 60 * 60)))
  //             .to.be.eq(totalPreUnlockSeed1.add(pricePerTranche));

  //           // 3M + 2M            
  //           const timeToTranche = (time) => ethers.BigNumber.from(time).div(6 * 60 * 60);
  ...
  //           // step1: claim in 3month (1week reward)
  //           // step2: claim in 3day (3day reward)
  ...
  //           const totalUnlockInClaim1 = pricePerTranche.mul((86400 * 7) / (6 * 60 * 60));
  //           expect(claims1, 'total unlocked after 1 week')
  //             .to.emit(ITO, 'OnClaim')
  ...
  //           const totalUnlockInClaim2 = pricePerTranche.mul((86400 * 3) / (6 * 60 * 60));
  ...
  //             .to.be.eq(preUnlock.add(pricePerTranche.mul((86400 * 10) / (6 * 60 * 60))));
  ...
  //           const totalUnlockInClaim3 = pricePerTranche.mul((86400 * 30) / (6 * 60 * 60));
  ...
  //             expect(newBalance3.sub(initBalanceSeed1), 'increase = prelock + 1 week 3 days + 1 month')
  //               .to.be.eq(preUnlock.add(pricePerTranche.mul((86400 * 40) / (6 * 60 * 60))));
